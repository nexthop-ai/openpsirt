...
| REQ-69 | Ingest is bounded — file size, nesting depth, component count — and every written field is length-bounded. No outbound request reaches an address inside this network or a host an administrator excluded | A scan file is hostile input, and the deployment sits inside somebody's network |
