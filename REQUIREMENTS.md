...
| **Users** | Staff, signed in through an OpenID Connect provider, GitHub, or a trusted proxy |
...
| # | Requirement | Why |
|---|---|---|
| REQ-05 | Takes in inventories pushed by build pipelines, one adapter per producer. A vulnerability report or a third party's VEX document may be uploaded alongside. CycloneDX, SPDX 2.x and SPDX 3.x are read | Both reference producers emit CycloneDX, so it was written first. An adapter per producer keeps one internal model, which is the seam the other two arrived through — and it held: nothing downstream of the reader knows which format a build sent |
...
| REQ-27 | One action may record the same judgment against many issues at one component, recording how the set was chosen. A bulk judgment is bounded by a configurable cap, because nothing re-checks it; a bulk promise to upgrade is not, because the next scan re-checks every row it names | A kernel carries 5,088 issues at 45 places each, and a bulk write with no cap is a denial of service somebody triggers by accident. That reasons from the dismissal, which nothing re-checks, so one sentence answering a thousand findings has to stay a size a reviewer can follow. A promise is the one bulk write that verifies itself, and narrowing one makes the record false, because the upgrade closes what it closes: one kernel bump reaches 44,016 places against a shipped cap of 2,000. The other half of what a cap was doing is transaction size, and the largest act the data can ask for, a promise over 243,950 places, commits in 8.3 seconds on SQLite, 8.4 on MariaDB, 10.9 on MySQL and 23.9 on PostgreSQL, so there is nothing a chunk or a queue would solve |
...
| REQ-29 | What is recorded against an issue — a rating that disagrees with the published one, or a note for whoever decides — belongs to one product, applies to every build of it, and does not lapse when a version moves. Two products may record different things about the same issue | A rating is a judgment about how something is used, and two products do not use a component the same way. A single rating let somebody holding one product set the deadline and the triage line in a product they cannot see, and refused a second team any opinion at all. A note is read by the same people under the same rule, so it is scoped the same way rather than by a second rule nobody could derive |
...
| REQ-31 | A supplier's or other third party's judgment is shown as evidence and offered as a prefill, and never decides anything by itself | "The distribution will not fix it" is not "we are not affected" |
...
| REQ-39 | An advisory is published about flaws in our own product, as a machine-readable document. A VEX document is generated per build from approved dismissals and from the patches the build declares. **Delivery adapters are not built** | Known issues in shipped third-party components are tracked and fixed, not published about — but VEX is precisely the document for them, and it does not drift from prose nobody regenerates |
...
| REQ-69 | Ingest is bounded — file size, nesting depth, component count — and every written field is length-bounded. Where a document chooses the host, as a supplier's description or a patch link does, the request reaches nothing inside this network and no host an administrator excluded. Every other outbound request goes only to a host an administrator configured or the application names | A scan file is hostile input, and the deployment sits inside somebody's network |
