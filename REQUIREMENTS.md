...
| # | Requirement | Why |
|---|---|---|
| REQ-05 | Takes in inventories pushed by build pipelines, one adapter per producer. A vulnerability report or a third party's VEX document may be uploaded alongside. CycloneDX and SPDX 2.x are read | Both reference producers emit CycloneDX, so it was written first. An adapter per producer keeps one internal model, which is the seam the second format arrived through — and it held: nothing downstream of the reader knows which format a build sent |
...
| REQ-76 | A document says which format it is and the reader is chosen from that, never from the request. A format's major version is read only where it has been written against; the third major version of SPDX is refused by name | An endpoint taking a format parameter is a parameter a build sets wrong, and a reader that guesses eventually guesses wrong on a file that looks close enough. SPDX 3.0 is a different document rather than a revision — one flat graph of typed elements sharing no key path with 2.x — so reading it on the assumption that it did not move is the failure refusing by name exists to prevent |
