...
| REQ-76 | A document says which format it is and the reader is chosen from that, never from the request. A major version is read only where it has been written against, and one that has not been is refused by name | An endpoint taking a format parameter is a parameter a build sets wrong, and a reader that guesses eventually guesses wrong on a file that looks close enough. A major version is where a format moves things: SPDX 3.x shares no key path with 2.x, so reading one on the assumption that it is the other is the failure refusing by name exists to prevent |
| REQ-78 | Where a format annotates a statement with something it does not define the meaning of, the reading that keeps a component is taken. A lifecycle scope says which phase a relationship matters in, and only a test scope stops the relationship placing its target under anything — the target is still held, and still counted as sitting under nothing | Reading a build-phase dependency as not shipping is wrong for every compiled language — a crate linked into a binary is stated that way and is inside what ships. The two errors are not equal: keeping too much adds something to triage, which somebody sees and acts on, and dropping too much removes a finding nobody ever learns about. The format's own example states one application's three dependencies unscoped in the second version and build-scoped in the third, so the document that loses everything is a real one |
| REQ-77 | What a format cannot state is recorded as a limit of the format, not filled in. An inventory carries the build's own claims about vulnerabilities only where the format can attach one to a component | SPDX can say a file is a patch for a package and cannot say which vulnerability the patch resolves, so a build sending that format states its carried patches in a document of its own. Inventing the link would report a suppression nobody made |
...
| REQ-79 | An object store reached in the clear is refused unless it is this machine or an operator has stated that network, and a deployment that states one is told at every start what it has exposed. See REQ-70 | An attachment is delivered as a redirect, so the signed address is a bearer token that travels to a browser and anybody on the path may spend it. The store somebody already runs is the constraint, and an installation that cannot put TLS in front of it should be able to run this rather than be told it has configured the tool wrongly — but not by accident, and not silently. What the setting exposes is invisible from the setting itself, which is why the cost is stated where a deployment starts rather than only where it was configured |
