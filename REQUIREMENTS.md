...
| REQ-25 | A decision survives re-scans and carries automatically to other releases, tags and variants whose chains and versions already match. A judgment about risk lapses when the software changes. A claim that the scanner matched something that is not there does not lapse, because it is a claim about identity | A decision that lapsed nightly would be re-made nightly, and the same finding across a dozen builds is one judgment. No version makes a wrong match right, so a claim about identity that expired on one would be answered again at every release |
