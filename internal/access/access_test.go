...
func TestSomebodyUnknownAndSomebodyUngrantedGetTheSameAnswer(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		if _, err := f.store.Ensure(ctx, "known", "Known Person", nil, nil); err != nil {
			t.Fatal(err)
		}
...
func TestARoleOnOneProductSaysNothingAboutAnother(t *testing.T) {
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		person, err := f.store.Ensure(ctx, "reader", "Reader", nil, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestReadingPublicIsNotReadingPrivate(t *testing.T) {
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		person, _ := f.store.Ensure(ctx, "public-only", "Public Only", nil, nil)
		if err := f.store.GrantRole(ctx, person.ID, f.products["sonic"], access.PublicRead); err != nil {
			t.Fatal(err)
		}
...
func TestACapabilityHandsOverNoVisibility(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		person, _ := f.store.Ensure(ctx, "approver", "Approver", nil, nil)
...
func TestAReportingRoleCannotBeGranted(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		person, _ := f.store.Ensure(ctx, "auditor", "Auditor", nil, nil)
...
func TestAnAdministratorAdministersRatherThanHoldingEveryRole(t *testing.T) {
	...
	each(t, func(t *testing.T, f *fixture) {
		ctx := t.Context()
		if _, err := f.store.Ensure(ctx, "admin", "Admin", access.Stated(true), nil); err != nil {
			t.Fatal(err)
		}
