...
func TestSomebodyJustAddedIsNotAlreadyAbsent(t *testing.T) {
	...
	dbtest.Each(t, func(t *testing.T, db *database.DB) {
		...
		fresh, err := rights.Ensure(ctx, "fresh@example.com", "Fresh Start", nil, nil)
		if err != nil {
			t.Fatal(err)
		}
...
func TestSomebodyAwayHoldingWorkIsRaisedAndAnIdleAccountIsNot(t *testing.T) {
	...
	dbtest.Each(t, func(t *testing.T, db *database.DB) {
		...
		away, err := rights.Ensure(ctx, "away@example.com", "Ana Away", nil, nil)
		if err != nil {
			t.Fatal(err)
		}
		// Away just as long, and holding nothing. An idle account is not an
		// alert; it is an account.
		idle, err := rights.Ensure(ctx, "idle@example.com", "Ivan Idle", nil, nil)
		if err != nil {
			t.Fatal(err)
		}
		// Here yesterday and holding work, which is somebody doing their job.
		here, err := rights.Ensure(ctx, "here@example.com", "Hana Here", nil, nil)
		if err != nil {
			t.Fatal(err)
		}
