...
var settable = []struct {
	...
	{setting.PatchBranches, "Patch branches", "outbound",
		"Fetch the repositories patch links point into, to label commits with their branches",
		"A fetch of the repository a report's link names, from any host outside this network that OPENPSIRT_OUTBOUND_EXCLUDED does not list.",
		aSwitch, theSwitch, func(Ingest) string { return setting.Off }, false},
