...
type PatchRepositoryBody struct {
	...
	State     string `json:"state" enum:"waiting,working,failed,done,excluded" doc:"'waiting' has not been visited. 'working' is a visit under way. 'failed' is a last visit that stopped on an error, retried a day after it began. 'done' is a last visit that finished. 'excluded' is on a host OPENPSIRT_PATCH_EXCLUDED lists, and is never fetched"`
