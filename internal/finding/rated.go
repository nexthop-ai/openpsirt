...
// list is mostly made of. The argument is where the query finds its product: a
// column it already carries — `st.product_id`, `de.product_id` — or `?` where
// the product is bound. It is written at the call site and never arrives from
// a request, which is the same rule every other identifier in a statement
// follows.
//
...
// where none existed.
func RatedFor(product string) string {
...
const RatedHere = `LEFT JOIN "issue_rating" AS ir ON ir.vulnerability_id = v.id` +
	` AND ir.product_id = ?`
