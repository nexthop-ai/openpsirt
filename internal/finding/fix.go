...
type Upgrade struct {
	...
	// a branch. Stored and never fetched — a link has no egress at all,
	// which is what makes it available to a deployment that has not
	// decided to let anything out.
...
// and reporting them alike is how the second gets lost among the first . A
// build that was chosen and no longer holds the issue is here too: that is the
// one that worked, and dropping it from the list would leave a finished piece
// of work looking like one nobody ever declared.
type Intent struct {
	TargetID int64
	Stream   string
	Variant  string
	...
	// Elsewhere is where that work is happening, where somebody said.
	Elsewhere string
	...
	// remediated, so it carries no target and is not counted as
	// outstanding .
	PastEndOfLife bool
}
...
// before it did. Nothing on it will be fixed, so counting it as outstanding
// fills the figure permanently and counting it as delivered claims a fix
// nobody shipped. It is neither, and it is still listed — as retired, so
...
// the issue.
//
// The scan is independent evidence against the claim. Without the "since" it
// would flag every declaration made between two nights, which is most of them
// and would make the flag worthless within a week.
...
//
// The other half of "gone from main, still present in 2.4 and 2.3". Derived
// only from scans: nobody claimed it, and a build that fixed something on the
// way past is still a build that fixed it.
...
// but it has to read as silence rather than as a plan.
...
// somebody choosing has in front of them, plus every build that once held it
// and no longer does — which is the other half of the same question: "gone
// from main, still present in 2.4 and 2.3".
//
...
func (s *Store) FixingIn(ctx context.Context, subject access.Subject,
	...
	// And where it ever was. A second pass rather than one query counting
	// both with a conditional sum: that sum comes back as a decimal on two
	// of the four engines, and the cast to make it an integer is
	// engine-specific spelling the core does not carry.
	var ever []int64
	err = s.db.NewSelect().
		TableExpr("finding AS f").
		Join("JOIN target AS tg ON tg.id = f.target_id").
	...
	declared, err := s.declaredFor(ctx, productID, componentID)
	if err != nil {
		return nil, err
	}
	...
	retired, err := s.retired(ctx, s.db, ids)
	if err != nil {
		return nil, err
	}
	...
	for _, id := range ids {
		...
		if row, ok := declared[id]; ok {
			...
			intent.Elsewhere = row.Elsewhere
...
// the one that says what the answer now is. Sending an empty set is how a plan
// is withdrawn.
//
...
// per build that exists.
func (s *Store) FixIn(ctx context.Context, subject access.Subject,
	productID, vulnerabilityID, componentID int64, builds []int64,
	elsewhere map[int64]string) (int, error) {
	...
	// One transaction, opened here because this is the whole act. Through
	// the one helper, so it is retried whole — and everything it decides
	// from is read inside it, because a retry runs against a database that
	// has moved. Which builds the names resolve to and which of them are
	// out of support are both such reads: taken before the transaction, a
	// release retired between attempts still looks supported, and the
	// refusal that exists to stop a fix being declared against one is the
	// very thing that gets skipped.
	var declared int
	err := database.InTransaction(ctx, s.db, func(ctx context.Context, tx bun.Tx) error {
		wanted, err := s.buildsOf(ctx, tx, productID, builds)
		if err != nil {
			return err
		}
		retired, err := s.retired(ctx, tx, builds)
		if err != nil {
			return err
		}
		// The version this moves to, read here while the findings are still
		// open. It cannot be recovered afterwards: landing the fix closes
		// them, and a plan that stops naming the version as the work succeeds
		// goes blank exactly when it is working.
		to, err := s.movingTo(ctx, tx, productID, vulnerabilityID, componentID)
		if err != nil {
			return err
		}
		declared, err = s.CommitWithin(ctx, tx, subject, productID, componentID,
			to, nil, nil, wanted, retired, elsewhere)
		return err
	})
	return declared, err
}
...
func (s *Store) CommitWithin(ctx context.Context, db bun.IDB, subject access.Subject,
	...
	wanted []int64, retired map[int64]bool, elsewhere map[int64]string) (int, error) {
	...
	for _, id := range wanted {
		...
		// today.
		if have[id] {
			// The link is the one part of a commitment that is meant to change
			// without ceremony: work moves to a different ticket, or nobody
			// had one when the plan was made. Written only where the request
			// said something, so a plan re-sent without links does not erase
			// them.
			if where, said := elsewhere[id]; said {
				if _, err := db.NewUpdate().Model((*Upgrade)(nil)).
					Set("elsewhere = ?", strings.TrimSpace(where)).
					Where("fold_key = ?", fold).
					Where("target_id = ?", id).Exec(ctx); err != nil {
					return 0, fmt.Errorf("record where the work is: %w", err)
				}
			}
		...
		// The same rule a link inside text goes through. A browser and the
		// page's policy both refuse a script scheme, and neither refuses a
		// custom application one — which hands a privileged person's click to
		// a program on their machine, and is the class this restriction exists
		// for everywhere else here.
		at := strings.TrimSpace(elsewhere[id])
...
// declaredFor reads the commitments covering one component, per build.
//
// Asked of the fold rather than of the component: a bump moves the source
// package, so the commitment recorded against curl covers libcurl4t64 as well.
func (s *Store) declaredFor(ctx context.Context, productID,
	componentID int64) (map[int64]Upgrade, error) {

	var rows []Upgrade
	err := s.db.NewSelect().Model(&rows).
		Where(`fold_key = (SELECT c.fold_key FROM "component" AS c WHERE c.id = ?)`, componentID).
		Where(`target_id IN (SELECT tg.id FROM "target" AS tg
			JOIN "stream" AS st ON st.id = tg.stream_id
			WHERE st.product_id = ?)`, productID).
		Scan(ctx)
	if err != nil {
		return nil, fmt.Errorf("read what is committed: %w", err)
	}
	declared := make(map[int64]Upgrade, len(rows))
	for _, row := range rows {
...
// movingTo is the upstream version that fixes this issue at this component, as
// the open findings report it.
//
// Read at the moment a fix is declared and stored with the declaration, for
// the reason a scan's provenance is stored: it describes a moment, and the
// moment is gone as soon as the fix lands and the findings close.
func (s *Store) movingTo(ctx context.Context, db bun.IDB,
	productID, vulnerabilityID, componentID int64) (string, error) {

	var to string
	err := db.NewSelect().
		TableExpr("finding AS f").
		ColumnExpr("COALESCE(MIN(f.fixed_in), '')").
		Where("f.vulnerability_id = ?", vulnerabilityID).
		Where("f.component_id = ?", componentID).
		Where("f.closed_at IS NULL").
		...
		Scan(ctx, &to)
	if err != nil {
		return "", fmt.Errorf("read what version fixes this: %w", err)
	}
	return to, nil
}
