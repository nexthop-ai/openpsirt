...
| `tern-photon-image.spdx.json` | Real output. Tern scanning a Photon OS 3.0 image, SPDX 2.2. Its 37 packages carry a name and a version and no package identifier or CPE, so nothing can match them: a build of these scans to zero findings. **CC0-1.0**, stated in the document. From [tern-tools/tern](https://github.com/tern-tools/tern) |
| `sbom-tool-linux.spdx.json` | Real output. Microsoft's sbom-tool describing its own 4.1.5 Linux release, SPDX 2.2. It points at other documents through `externalDocumentRefs` and states `DESCRIBED_BY`. **CC0-1.0**, stated in the document. From the release assets of [microsoft/sbom-tool](https://github.com/microsoft/sbom-tool) |
| `apko-wolfi-image.spdx.json` | Real output. apko describing a Chainguard Wolfi Python image, SPDX 2.3, taken from the image's signed attestation. Each package is described three times: two APK entries that identity folds into one, and a directory entry with no package identifier. Its root is a layer digest. **CC0-1.0**, stated in the document |
| `oe-linked.spdx.json` | Real output. OpenEmbedded's `create-spdx` class, SPDX 2.2: one document of a set that link to each other through `externalDocumentRefs`, stating no `DESCRIBES`, so the build stands in as its root. **CC0-1.0**, stated in the document. From the OpenChain Telco working group's examples |
| `sbom-tool-hello.spdx3.json` | The specification's own example 12, at SPDX 3.0.1, written by sbom-tool. Its only package is the document's subject, so it holds nothing to scan. CC0-1.0, as the other SPDX examples |
| `maven-dropwizard.cdx.json` | Real output. The CycloneDX Maven plugin 2.0.2 describing Dropwizard 1.3.15, **CycloneDX 1.2**, the oldest revision here. Its dependency graph has 29 roots besides the document's own, which hang from the root. **CC0-1.0**, from [CycloneDX/bom-examples](https://github.com/CycloneDX/bom-examples) |
| `saasbom-services.cdx.json` | A SaaSBOM from [CycloneDX/bom-examples](https://github.com/CycloneDX/bom-examples): services and data flows, and no components at all. What is read is the root alone. **CC0-1.0** |
| `trivy-ubuntu.cdx.json` | Real output. Trivy 0.48 scanning `ubuntu:latest`, CycloneDX 1.5, carrying the producer's own vulnerabilities, which are not read: vulnerability data is produced here. **BSD-3-Clause**, from the DefectDojo test data, which `NOTICE` records |
| `snyk-juice-shop.cdx.json` | Real output. Snyk's SBOM export of OWASP Juice Shop, CycloneDX 1.4, 1,004 components with dependency chains sixteen deep, the deepest among 41 public inventories. **MIT**, which `NOTICE` records |
| `scout-python-image.spdx.json.xz` | Real output. BuildKit and Docker Scout describing `python:3.13-slim`, SPDX 2.3, taken from the image's attestation. Its package identifiers state the distribution as `os_name` and `os_version` rather than `distro`, which is what the scanner reads. **CC0-1.0**, stated in the document. Compressed |
| `github-dependency-graph.spdx.json.xz` | Real output. GitHub's dependency graph export of grafana/loki, SPDX 2.3: 545 packages and 5,075 `DEPENDS_ON` relationships. **CC0-1.0**, stated in the document. Compressed |
