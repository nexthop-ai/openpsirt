...
func init() {
	goose.AddMigrationContext(upDisclosureMovement, downDisclosureMovement)
}
...
func upDisclosureMovement(ctx context.Context, tx *sql.Tx) error {
	t, err := types(ctx)
	if err != nil {
		return err
	}
...
func downDisclosureMovement(ctx context.Context, tx *sql.Tx) error {
	return dropTables(ctx, tx, "disclosure_movement")
}
