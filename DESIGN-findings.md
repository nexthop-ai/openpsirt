...
beside the supplier, so a build can be asked which licenses it ships. Measured on
a switch image: 4,483 of 6,866 components declare one.
...
| Several licenses listed for one component are one conjunction | A package listing several is under all of them. Joined with AND, a compound one parenthesized, repeats dropped, in the producer's order |
