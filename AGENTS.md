...
| **Request forgery** | A fetch to an address that arrived from outside — a sign-in provider's discovery, a package index, a patch link's repository, a supplier's directory — reaches no address inside this network, checked at connect time on the address a name resolved to, and follows no redirect (REQ-69). Webhooks, mail and the object store go where an administrator configured them, private addresses included, and their design documents say so. Two fetches go to hosts somebody outside chose: the repository a patch link names, through the loopback proxy (REQ-78), and a supplier's directory and documents, wherever its description names them. For those two the excluded list is all that keeps out an internal service on a public name. A fetch whose host is fixed — sign-in, the package indexes — stays pinned to it |
