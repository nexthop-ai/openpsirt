...
| **Request forgery** | Where a document chooses the host — the repository a patch link names, through the loopback proxy (REQ-78), and a supplier's directory and documents, wherever its description names them — the request reaches nothing inside this network and no host on the administrator's excluded list, checked at connect time on the address a name resolved to, and follows no redirect. For those two the excluded list is all that keeps out an internal service on a public name. Every other outbound request goes only to a host an administrator configured or the application names: sign-in and the package indexes stay pinned to theirs, and webhooks, mail and the object store go where they were configured, private addresses included (REQ-69) |
