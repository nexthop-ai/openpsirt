...
| Every person in the `internal/httpapi` fixtures is seeded with no display name | So the identity and the label coincide, and a field publishing the wrong one of the two cannot be told from a field publishing the right one. The routes that round-trip a name — the collaborator list, the team members, and the role, credential and token listings — now carry the address in the field a write resolves and the label beside it, each pinned by a person whose two names differ. Giving the rest of the fixture people names is what would make the remaining fields checkable, and it is deliberately not done until they are settled one at a time |
