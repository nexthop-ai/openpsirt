...
| **No publisher's real security advisory is in the test corpus** | The four fixtures are written here, in the shapes four real advisories use, because none of the four carries a license this tree can take: three assert copyright and grant nothing, and the fourth publishes a `LICENSE` granting CC-BY-4.0 beside its directory. CC-BY-4.0 is a real permissive grant and is not on `ALLOWED_LICENSES`, which REQ-01 is about. Adding it for a data file, with attribution in `NOTICE`, would buy back the one thing writing them here gives up — that somebody else chose the cases — for the one publisher where it can be bought |
