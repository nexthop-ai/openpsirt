...
| Nothing fetches a supplier's advisories | They arrive by administrator upload. A publisher issues one advisory per issue, so keeping up with a directory by hand is a pass somebody does or forgets, and `internal/outward` is already the client for it — https only, no redirects, refusing an unconfigured host |
...
| **An issue carries no published or modified date** | Neither is in what the scanner reports: its vulnerability record has a description, an advisory link, the scores, the estimate and the weaknesses. Recording them means a second source — the issue's own record, fetched — which is a capability rather than a field, and this project fetches nothing from a feed today |
