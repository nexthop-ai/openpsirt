...
| Version ordering for NuGet | It is SemVer 2.0 with a fourth numeric segment permitted, so it is not the scheme the language ecosystems share and needs its own. The long tail behind it — RubyGems, Composer, Hex, Pub, SwiftPM, Conan — is not deferred work: coverage can never be complete for a product whose deployers' ecosystems are unknowable, and an unranked list is the honest answer for them |
