...
| The SPDX 3.x reader is proved by the specification's examples alone | Every other format here is read against a producer's own output as well. Nothing this deployment ingests emits 3.x yet — the scanner it ships emits 2.3 and tag-value — so the four fixtures are the specification's, which are hand-written and small by construction. Yocto and one vendor tool emit it, so the missing evidence is gettable rather than hypothetical, and what it would settle is which shapes a real producer actually uses |
