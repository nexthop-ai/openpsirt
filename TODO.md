...
| An outsized inventory delta raises nothing, and nothing lists what changed | A receipt says how many component names an upload added, removed and moved. Nothing notices that the number is unlike this build's others, and nothing shows which names they were. The two are one piece of work rather than two: an alert whose investigation path does not exist is an alarm pointing at nothing. The threshold is per deployment, so it is a setting something reads rather than a constant |
