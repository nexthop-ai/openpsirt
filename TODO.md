...
| Four read routes publish a display name where a sign-in identity is documented | The trail, the collaborator list, the role listing and the finding detail all resolve a person through the store's presentation helper, which answers a display name where one is known and the identity otherwise, and each publishes it in a field the API description calls a sign-in identity. A list is then not usable to undo what it shows. The test fixtures in `internal/httpapi` seed every person with no display name, so the two coincide and no test can see it — giving those people names is the first half of the fix and is deliberately not done until the fields are settled one at a time |
