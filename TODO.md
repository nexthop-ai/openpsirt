...
| Neither kind of publisher document can be uploaded from a screen | Both routes are an API act, so an administrator holding a document a distribution published has to reach for a request builder — which is where the evidence layer stops being used. The row below is about a document going out rather than one coming in |
...
| A flaw already fixed cannot be picked from the advisory's list | The list of what to name on an advisory reads `/v1/products/{product}/findings`, which answers what is open and says so in its own description. An advisory written after the fix landed gets an empty picker for a product that does hold the flaw. The server takes any flaw recorded here, so typing the identifier still reaches it, and the screen says the list is what is still open. Widening the list is a server change: what a closed row is doing in a list built around triage is the question, rather than a filter somebody adds |
...
| Version ordering for NuGet | It is SemVer 2.0 with a fourth numeric segment permitted, so it is not the scheme the language ecosystems share and needs its own |
...
| No gate sees the license of a file vendored into the tree | `make licenses` walks Go modules and the dependency review reads manifests, so a version suite, a weakness catalog, a specification's example document or a publisher's advisory copied into the tree is checked by nobody. Each is recorded in `NOTICE` by hand, which is a habit rather than a control. What would close it is a gate over what is vendored rather than what is imported, and the hard half is that a data file states its license in its own words where it states one at all |
| A report can be judged only by pointing it at an issue | The five outcomes a claim can be given — accepted, duplicate, out of scope, not reproducible, rejected — are not built, and neither is the second person that rejecting one or declaring it out of scope requires: rejecting a report plainly hides risk, and the bulk cap REQ-27 sets is what makes answering twenty slop reports in one act bearable. Until they exist a claim that is not a flaw is answered, left unjudged and filed, which records what happened and does not say what was decided. No screen reaches a report either, because an inbox without the outcomes is one that gets rebuilt beside them |
| Nothing says a supplier has stopped answering | A configured publisher that has been unreachable for a week goes on looking exactly like one that has published nothing, which is the silent failure the system screen exists for. The moment of the last attempt and the reason it stopped are recorded and shown on the configuration panel, so what is missing is the condition and the place it is read — the webhook delivery panel is the shape |
| A publisher's history is not fetched | Reading starts at the moment a supplier is configured, because a distribution's feed lists every advisory they have ever issued and taking it is tens of thousands of requests draining over months. An older one is taken by uploading it. What would close it is deciding what a bounded backfill is worth asking a publisher for |
...
| A VEX document can state a version the record does not agree with | The document is generated outside the write that records the issuance, so a lost race for the ordinal hands the operator a document numbered one behind what it was recorded as, and the revision in between never exists in the wild. The advisory's twin is answered by keeping the bytes that went out; a VEX document keeps none, and what it would take is deciding what to do with a document somebody has already been handed |
...
as though none of it is urgent. **Two of them are behavior that is built and a row
...
| **REQ-23 names a fixed set of outcomes and the code records one more** | Built, and the row is short by a word. A wrong match is its own outcome, and whether REQ-23's enumeration is broadened to name it is the owner's. `DESIGN-triage.md` § Corrections holds what one is |
