...
| An algorithm is transcribed from the project that defines it, and checked against that project's own vectors | The tilde and caret rules, a numeric run outranking an alphabetic one, a leading zero turning a version part into text: each is a decision somebody made rather than something to reason out from a description. A suite is committed beside the code, verbatim, and a reading that is nearly right fails against it |
| A scheme is added by adding its algorithm, never by mapping it onto one already here | Each of the four is unlike the others in some detail that only shows on a pair nobody thought to try. Sharing a scheme is right where an ecosystem genuinely uses it — three language ecosystems do — and elsewhere it is the confident wrong answer wearing a familiar name |
