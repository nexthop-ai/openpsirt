...
| Nothing is withheld for sitting past the named fix | It would be the right rule and is not enforced: deciding whether one version sits past another needs an ordering for the ecosystem in hand, and there is one for four of them and not for the rest — a rule that applied to some ecosystems and not others would be worse than none. What is recorded instead is the case that needs no ordering — a version moved and the issue came with it |
