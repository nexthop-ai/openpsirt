...
| Nothing lists what a text refers to, either | Reading the list back is how a resolver would be fed, and there is no resolver. A function that produced one sat exported and uncalled, which reads as a mechanism somebody can rely on. Whoever renders the text finds the references in it, because they are in the text |
