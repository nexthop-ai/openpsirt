...
| Rule | |
|---|---|
| The queue, the jobs it gave up on, whether the webhooks are arriving, and what asking upstream could not answer, on one screen | They are one question — is this deployment working — and each of them fails the same way. A queue that has given up looks exactly like a quiet one, a webhook refusing every request for a week looks exactly like one nothing has been sent to, and a component held back looks exactly like one no index has heard of |
