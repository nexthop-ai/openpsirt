...
| **A row is remembered by what it is, not by its name** | The name, the version, the kind of package and its namespace together. What is open, what has already been drawn, and what sits under each node are all held against that — and so is the request for a node's children, which is refused for a name that means two things. Keyed on the name, a component the build ships twice could not be opened at all |
