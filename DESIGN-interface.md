...
| A variant is corrected and retired from the product's own list | An administrator edits its name and whether it ships, or takes it out of use, from the row. The release list is what a release was built as rather than what the product declares, so it carries no control: correcting one thing in two places is how the two come to disagree |
