...
| The daily questions are on the bar | Decision state, severity, who holds it and when it is due, each a button naming itself and its value, with New today beside them. Everything else is behind More filters, a panel that opens with how many of its own filters are on written on the control while it is shut. A filter is on the bar or in the panel, never both |
...
| A tab says how many of its settings differ from the shipped value | A value changed on a tab nobody opened is otherwise invisible. A tab with none changed says how many it holds |
